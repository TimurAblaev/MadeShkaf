class SLIDER {
    constructor(el){
      ...
      this.isDots = el.isDots
      this.interval = this.timeMove + 1000
      // dots
      if(this.isDots == 'true' || this.isDots == null) {
        let isDots = this.slider.getAttribute('dots')
        ...
        this.active = true
        // autoplay
        // let isAutoplay = this.slider.getAttribute('autoplay');
        if(this.autoplay == 'true' || this.autoplay == null) {
  ...
  sliders.forEach((item, key)=>{
    ...
    const autoplay = item.getAttribute('autoplay')
    const isDots = item.getAttribute('dots')
